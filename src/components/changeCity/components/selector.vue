...
<template>
  <div class='choose-wrap' :class="disabled?'disabled':''" @click="showWrapper" v-document-click="cityDomClick">
    <div class="choose" >
      <span>{{ value }}</span>
      ...
      <div :class="{ 'mt-content': true, active: isShowWrapperActive }">
        ...
        <div class="wrapper">
          <div class="col" v-for="(item, index) in twelveArr" :key="index">
            <span :class="{'mt-item': true, 'active': j.name === value}"  v-for="(j, i) in item" :key="i" @click="changeCityAndProvince(j)">{{j.name}}</span>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  props: ['list', 'title', 'value', 'isShowWrapperActive', 'disabled'], // isShowWrapperActive: 下拉菜单是否显示 默认不显示
  methods: {
    // #点击城市按钮显示下拉菜单
    showWrapper (e) {
      e.stopPropagation()
      if (!this.disabled) {
        ...
        console.log('未选择省份,城市列表点击失效 / 用户不能点击')
      }
    },
    cityDomClick () {
